...
from docling_core.types.doc.labels import DocItemLabel, GroupLabel, TableCellLabel, PictureClassificationLabel
...
def find_box(boxes: List, point: Tuple[float, float]):
    ...
    if index==-1:
        ...
        for i,box in enumerate(boxes):
            ...
            logging.infor(f"=> bbox: {l:.3f}, {r:.3f}, ({(l<x) and (x<r)}), {t:.3f}, {b:.3f}, ({(t<y) and (y<b)})")
...
def parse_annotation(image_annot:dict):
    ...
    if "box" not in image_annot or "polyline" not in image_annot:
        return basename, keep, boxes, lines, reading_order, to_captions, to_footnotes, next_text
    ...
    return basename, keep, boxes, lines, reading_order, to_captions, to_footnotes, to_values, merges, group
...
def get_next_provs(page_no:int,
                   boxid:int,
                   text:str,
                   boxes: list,
                   merges:list,
    ...
    for merge in merges:
        if len(merge["boxids"])>1 and merge["boxids"][0]==boxid:
            ...
            for l in range(1, len(merge["boxids"])):
                ...
                label_, prov_, text_ = get_label_prov_and_text(box=boxes[boxid_],
                                                               page_no=page_no,
...
def add_captions_to_item(to_captions:list,
                         item: FloatingItem,
                         page_no:int,
                         boxid:int,
                         boxes:list,
    ...
    for to_caption in to_captions:
        if to_caption["boxids"][0]==boxid:
            for l in range(1, len(to_caption["boxids"])):
                ...
                label, prov, text = get_label_prov_and_text(box=caption_box,
                                                            page_no=page_no,
                                                            img_width = true_doc.pages[page_no].image.size.width,
                                                            img_height = true_doc.pages[page_no].image.size.height,
                                                            pdf_width = true_doc.pages[page_no].size.width,
...
def add_footnotes_to_item(to_footnotes:list,
                          item: FloatingItem,
                          page_no:int,
                          boxid:int,
                          boxes:list,
    ...
    for to_footnote in to_footnotes:
        if to_footnote["boxids"][0]==boxid:
            for l in range(1, len(to_footnote["boxids"])):
                ...
                label, prov, text = get_label_prov_and_text(box=footnote_box,
                                                            page_no=page_no,
                                                            img_width = true_doc.pages[page_no].image.size.width,
                                                            img_height = true_doc.pages[page_no].image.size.height,
                                                            pdf_width = true_doc.pages[page_no].size.width,
...
def create_true_document(basename:str, annot: dict, desc: dict):
    ...
    _, keep, boxes, lines, reading_order, to_captions, to_footnotes, to_values, merges, group = parse_annotation(annot)
    assert _==basename
    ...
    for boxid in reading_order["boxids"]:
        ...
        label, prov, text = get_label_prov_and_text(box=boxes[boxid],
                                                    page_no=page_no,
                                                    img_width = true_doc.pages[page_no].image.size.width,
                                                    img_height = true_doc.pages[page_no].image.size.height,
                                                    pdf_width = true_doc.pages[page_no].size.width,
        ...
        next_provs, text, already_added = get_next_provs(page_no = page_no,
                                                         boxid = boxid,
                                                         text = text,
                                                         boxes = boxes,
                                                         merges = merges,
        ...
        if label in [DocItemLabel.TEXT,
            ...
            true_doc, already_added = add_footnotes_to_item(to_footnotes = to_footnotes,
                                                            item = table_item,
                                                            page_no = page_no,
                                                            boxid = boxid,
                                                            boxes = boxes,
            ...
            crop_image = crop_bounding_box(page_image=page_image, page=true_doc.pages[page_no], bbox=prov.bbox)
    ...
    for annot_file in annotation_filenames:
        ...
        for image_annot in annot_data["annotations"]["image"]:
            ...
            if basename not in overview:
                ...
                true_doc = create_true_document(basename=basename, annot=image_annot, desc=overview[basename])
                yield overview[basename], true_doc

def parse_args():
    parser = argparse.ArgumentParser(description="Create new evaluation dataset using annotation file.")

    parser.add_argument("-i", "--input_dir", required=True, help="Path to the input directory")
    parser.add_argument("-a", "--annot_file", required=False, help="Path to the CVAT annotation file.",
                        default="annotations.xml")

    args = parser.parse_args()
    return Path(args.input_dir), Path(args.input_dir) / args.annot_file,
