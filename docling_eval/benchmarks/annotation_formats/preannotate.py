...
from docling_eval.benchmarks.annotation_formats.utils import DocLinkLabel, TableComponentLabel
...
def create_cvat_preannotation_file_for_single_page(docs: List[DoclingDocument],
                                                   overview: List[dict],
                                                   output_dir:Path,
                                                   imgs_dir:Path,
                                                   page_imgs_dir:Path):
...
def export_from_dataset_supplementary_files(idir: Path,
                                            imgs_dir:Path,
                                            pdfs_dir:Path,
                                            json_true_dir:Path,
                                            json_pred_dir: Path):
...
def parse_args():
    parser = argparse.ArgumentParser(description="Process input and output directories and a pre-annotation file.")

    parser.add_argument("-i", "--input_dir", required=True, help="Path to the input dataset directory with parquet files.")
    parser.add_argument("-o", "--output_dir", required=True, help="Path to the output directory.")
    parser.add_argument("-a", "--preannot_file", required=False, help="Path to the pre-annotation file.", default="cvat-pre-annotations.xml")
    parser.add_argument("-p", "--project_file", required=False, help="Path to the project descriptiom file.", default="cvat-project.json")
    
    args = parser.parse_args()
    return Path(args.input_dir), Path(args.output_dir), Path(args.output_dir) / args.preannot_file, Path(args.output_dir) / args.project_file
...
def main():        
    ...
    for _ in [output_dir, imgs_dir, page_imgs_dir, pdfs_dir, json_true_dir, json_pred_dir, json_anno_dir]:
        os.makedirs(_, exist_ok=True)
    
    create_cvat_project_properties(project_file=project_file)
    
    docs, overview = export_from_dataset_supplementary_files(idir=input_dir,
                                                             pdfs_dir=pdfs_dir,
                                                             imgs_dir=imgs_dir,
    ...
    create_cvat_preannotation_file_for_single_page(docs=docs, overview=overview,
